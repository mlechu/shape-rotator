...
op_ambo_face_face(F0, F) :-
    ...
    l_map(call(ap_make_e_r), VAs, VBs, Es),
    F = face(Es).
...
op_ambo(AP0, AP) :-
    AP0 = ap(F0s),
    %% One vertex for each old edge
    %% setof(E0_tmp, ap_has_e(AP0, E0_tmp), E0s),
    %% l_map(E0s, ap_edge_avg, Vs),
    %% l_zip(E0s, Vs, E0_V_mapping),
...
%% from there to each of the face's existing edges, hitting it slightly
%% counter-clockwise of the existing edge's midpoint.
%%
